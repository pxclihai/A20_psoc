...
UNS8 proceedSDO (CO_Data* d, Message *m)
{
    ...
    if (!err) {
        if ((whoami == SDO_SERVER) && (d->transfers[line].state == SDO_BLOCK_DOWNLOAD_IN_PROGRESS) ||(whoami == SDO_CLIENT) && (d->transfers[line].state == SDO_BLOCK_UPLOAD_IN_PROGRESS))
        {
        ...
        UNS8 subIndex, UNS32 count, UNS8 dataType, void *data, SDOCallback_t Callback, UNS8 endianize, UNS8 useBlockMode)
{
    ...
    if(ret == 0xFE)
    {
        ...
        while (offset <= lastIndex)
        {
            if (d->objdict[offset].bSubCount <= 3)
            {
                MSG_ERR(0x1AC8, "Subindex 3  not found at index ", 0x1280);
                return 0xFF;
            }
            nodeIdServer = *(UNS8*) d->objdict[offset].pSubindex[3].pObject;
            if(nodeIdServer == 0)
