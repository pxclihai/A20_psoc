...
void CAN_1_ReceiveMsg(uint8 rxMailbox) 
{
        ...
        uint8 receiveData[8]; 
        uint8 index;   /* Defines 8 bytes of data */    
        CAN_message.rtr = 0;   /* Extended ID is not used */     
        CAN_message.irq = 1;      /* Writes address of structure that represents data*/ 
        CAN_message.msg = &Data;
    
    
        CAN_message.dlc = CAN_1_GET_DLC(rxMailbox);
            
        CAN_message.id = CAN_1_GET_RX_ID(rxMailbox);
            
        CAN_message.ide=CAN_1_GET_RX_IDE(rxMailbox);
            
        for (index=0;index<CAN_message.dlc ;index++)
        {
            receiveData[index] = CAN_1_RX_DATA_BYTE(rxMailbox,index); 
        }
      ...
      //  CAN_1_SendMsg(&CAN_message);
        ...
        m.cob_id=CAN_message.id;
        m.rtr=0;
        m.len=CAN_message.dlc;  
        for(index = 0; index < CAN_message.dlc; index++)
        m.data[index]=Data.byte[index];
