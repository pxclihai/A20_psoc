...
CY_ISR(RX_ISR_Interrupt)
{
    ...
    uint8 ch[20]={0};
    uint8 i;
        if(UART_1_GetRxBufferSize()>0 && UART_1_GetRxBufferSize()<=sizeof(ch))
        {
            for(i=0;i<UART_1_GetRxBufferSize();i++)
            {
                ch[i]=UART_1_ReadRxData();
            }
        }
    UART_1_PutArray(ch,sizeof(UART_1_GetRxBufferSize()));
