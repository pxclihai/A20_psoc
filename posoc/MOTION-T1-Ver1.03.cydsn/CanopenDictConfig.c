...
{
    MSG_USER(0, "Slave_heartbeatError %d\n", heartbeatID);
}
...
void Slave_initialisation(CO_Data* d)
{
	  ...
	  writeLocalDict( d, /*CO_Data* d*/
			 0x1017, /*UNS16 index*/
			 0x00, /*UNS8 subind*/
			 &HeartTimePeriod, /*void * pSourceData,*/
...
//    MSG_USER(0, "Slave_initialisation",0);
...
void Slave_preOperational(CO_Data* d)
{
     ...
     MSG_USER(0, "Slave_preOperational",0);
}
/**
  * @brief  Slave_preOperational.
  * @param  d:CANOpen object dictionary.
...
//	setTimer(100);
////	TIM_Cmd(TIM4, ENABLE);
    MSG_USER(0x00, "--------------------------------Slave_operational---------------------",0);
}
/**
  * @brief  Slave_stopped.
  * @param  d:CANOpen object dictionary.
...
{
    //MSG_USER(0, "Slave received EMCY message. Node: %2.2x  ErrorCode: %4.4x  ErrorRegister: %2.2x\n", nodeID, errCode, errReg);
    MSG_USER(0, "Slave received EMCY message. Node: %2.2x  ErrorCode: %4.4x  ErrorRegister: %2.2x\n", nodeID);
}
/**
...
{
        MSG_USER(0, "Slave_post_SlaveBootup %d", nodeid);
}
...
void Slave_post_SlaveStateChange(CO_Data* d, UNS8 nodeId, e_nodeState newNodeState)
{
        ...
        if (newNodeState == Operational)
        {
                ...
                if (NodeIdNum == MAX_CAN_BUS_ID)
                {
                        ...
                        MSG_USER(0x0000,"ENTER Slave_Operational",0);
                }
        }
}
...
//
//	initTimer();
//	CANOpenSlaveObject->canHandle = CAN1;

//	setState(CANOpenSlaveObject,Initialisation);
//	setNodeId (CANOpenSlaveObject, 127);
    CANOpenSlaveInit(&TestSlave_Data);
