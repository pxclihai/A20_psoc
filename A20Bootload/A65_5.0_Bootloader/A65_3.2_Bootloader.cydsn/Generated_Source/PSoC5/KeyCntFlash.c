...
/*******************************************************************************
* Function Name: KeyCntFlash_Start
********************************************************************************
*
* Summary:
...
void KeyCntFlash_Start(void) 
{
    ...
    KeyCntFlash_CLKEN |= KeyCntFlash_CLKEN_MASK;
	KeyCntFlash_CLKSTBY |= KeyCntFlash_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: KeyCntFlash_Stop
********************************************************************************
*
* Summary:
...
void KeyCntFlash_Stop(void) 
{
    ...
    KeyCntFlash_CLKEN &= (uint8)(~KeyCntFlash_CLKEN_MASK);
	KeyCntFlash_CLKSTBY &= (uint8)(~KeyCntFlash_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: KeyCntFlash_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((KeyCntFlash_CLKEN & KeyCntFlash_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = KeyCntFlash_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = KeyCntFlash_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* KeyCntFlash__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(KeyCntFlash_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(KeyCntFlash_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: KeyCntFlash_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: KeyCntFlash_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = KeyCntFlash_GetSourceRegister();
    uint16 oldDivider = KeyCntFlash_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = KeyCntFlash_CLKEN & KeyCntFlash_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((KeyCntFlash_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: KeyCntFlash_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(KeyCntFlash_DIV_PTR);
}


/*******************************************************************************
* Function Name: KeyCntFlash_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    KeyCntFlash_MOD_SRC |= modeBitMask & (uint8)KeyCntFlash_MODE_MASK;
}


/*******************************************************************************
* Function Name: KeyCntFlash_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    KeyCntFlash_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(KeyCntFlash_MODE_MASK));
}


/*******************************************************************************
* Function Name: KeyCntFlash_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return KeyCntFlash_MOD_SRC & (uint8)(KeyCntFlash_MODE_MASK);
}


/*******************************************************************************
* Function Name: KeyCntFlash_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = KeyCntFlash_GetDividerRegister();
    uint8 oldSrc = KeyCntFlash_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        KeyCntFlash_MOD_SRC |= CYCLK_SSS;
        KeyCntFlash_MOD_SRC =
            (KeyCntFlash_MOD_SRC & (uint8)(~KeyCntFlash_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: KeyCntFlash_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return KeyCntFlash_MOD_SRC & KeyCntFlash_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: KeyCntFlash_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    KeyCntFlash_PHASE = clkPhase & KeyCntFlash_PHASE_MASK;
}


/*******************************************************************************
* Function Name: KeyCntFlash_GetPhase
********************************************************************************
*
* Summary:
...
{
    return KeyCntFlash_PHASE & KeyCntFlash_PHASE_MASK;
}
