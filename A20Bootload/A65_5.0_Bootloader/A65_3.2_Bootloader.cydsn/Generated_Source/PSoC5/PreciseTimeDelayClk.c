/*******************************************************************************
* File Name: PreciseTimeDelayClk.c
* Version 2.0
*
*  Description:
...
/*******************************************************************************
* Function Name: PreciseTimeDelayClk_Start
********************************************************************************
*
* Summary:
...
void PreciseTimeDelayClk_Start(void) 
{
    ...
    PreciseTimeDelayClk_CLKEN |= PreciseTimeDelayClk_CLKEN_MASK;
	PreciseTimeDelayClk_CLKSTBY |= PreciseTimeDelayClk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: PreciseTimeDelayClk_Stop
********************************************************************************
*
* Summary:
...
void PreciseTimeDelayClk_Stop(void) 
{
    ...
    PreciseTimeDelayClk_CLKEN &= (uint8)(~PreciseTimeDelayClk_CLKEN_MASK);
	PreciseTimeDelayClk_CLKSTBY &= (uint8)(~PreciseTimeDelayClk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: PreciseTimeDelayClk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((PreciseTimeDelayClk_CLKEN & PreciseTimeDelayClk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = PreciseTimeDelayClk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = PreciseTimeDelayClk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* PreciseTimeDelayClk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(PreciseTimeDelayClk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(PreciseTimeDelayClk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: PreciseTimeDelayClk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: PreciseTimeDelayClk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = PreciseTimeDelayClk_GetSourceRegister();
    uint16 oldDivider = PreciseTimeDelayClk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = PreciseTimeDelayClk_CLKEN & PreciseTimeDelayClk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(PreciseTimeDelayClk_DIV_PTR, clkDivider);
                PreciseTimeDelayClk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                PreciseTimeDelayClk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(PreciseTimeDelayClk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((PreciseTimeDelayClk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(PreciseTimeDelayClk_DIV_PTR, clkDivider);
				PreciseTimeDelayClk_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: PreciseTimeDelayClk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(PreciseTimeDelayClk_DIV_PTR);
}


/*******************************************************************************
* Function Name: PreciseTimeDelayClk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    PreciseTimeDelayClk_MOD_SRC |= modeBitMask & (uint8)PreciseTimeDelayClk_MODE_MASK;
}


/*******************************************************************************
* Function Name: PreciseTimeDelayClk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    PreciseTimeDelayClk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(PreciseTimeDelayClk_MODE_MASK));
}


/*******************************************************************************
* Function Name: PreciseTimeDelayClk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return PreciseTimeDelayClk_MOD_SRC & (uint8)(PreciseTimeDelayClk_MODE_MASK);
}


/*******************************************************************************
* Function Name: PreciseTimeDelayClk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = PreciseTimeDelayClk_GetDividerRegister();
    uint8 oldSrc = PreciseTimeDelayClk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        PreciseTimeDelayClk_MOD_SRC |= CYCLK_SSS;
        PreciseTimeDelayClk_MOD_SRC =
            (PreciseTimeDelayClk_MOD_SRC & (uint8)(~PreciseTimeDelayClk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: PreciseTimeDelayClk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return PreciseTimeDelayClk_MOD_SRC & PreciseTimeDelayClk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: PreciseTimeDelayClk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    PreciseTimeDelayClk_PHASE = clkPhase & PreciseTimeDelayClk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: PreciseTimeDelayClk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return PreciseTimeDelayClk_PHASE & PreciseTimeDelayClk_PHASE_MASK;
}
