...
/*******************************************************************************
* Function Name: KeyScanClk_Start
********************************************************************************
*
* Summary:
...
void KeyScanClk_Start(void) 
{
    ...
    KeyScanClk_CLKEN |= KeyScanClk_CLKEN_MASK;
	KeyScanClk_CLKSTBY |= KeyScanClk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: KeyScanClk_Stop
********************************************************************************
*
* Summary:
...
void KeyScanClk_Stop(void) 
{
    ...
    KeyScanClk_CLKEN &= (uint8)(~KeyScanClk_CLKEN_MASK);
	KeyScanClk_CLKSTBY &= (uint8)(~KeyScanClk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: KeyScanClk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((KeyScanClk_CLKEN & KeyScanClk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = KeyScanClk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = KeyScanClk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* KeyScanClk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(KeyScanClk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(KeyScanClk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: KeyScanClk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: KeyScanClk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = KeyScanClk_GetSourceRegister();
    uint16 oldDivider = KeyScanClk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = KeyScanClk_CLKEN & KeyScanClk_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((KeyScanClk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: KeyScanClk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(KeyScanClk_DIV_PTR);
}


/*******************************************************************************
* Function Name: KeyScanClk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    KeyScanClk_MOD_SRC |= modeBitMask & (uint8)KeyScanClk_MODE_MASK;
}


/*******************************************************************************
* Function Name: KeyScanClk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    KeyScanClk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(KeyScanClk_MODE_MASK));
}


/*******************************************************************************
* Function Name: KeyScanClk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return KeyScanClk_MOD_SRC & (uint8)(KeyScanClk_MODE_MASK);
}


/*******************************************************************************
* Function Name: KeyScanClk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = KeyScanClk_GetDividerRegister();
    uint8 oldSrc = KeyScanClk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        KeyScanClk_MOD_SRC |= CYCLK_SSS;
        KeyScanClk_MOD_SRC =
            (KeyScanClk_MOD_SRC & (uint8)(~KeyScanClk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: KeyScanClk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return KeyScanClk_MOD_SRC & KeyScanClk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: KeyScanClk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    KeyScanClk_PHASE = clkPhase & KeyScanClk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: KeyScanClk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return KeyScanClk_PHASE & KeyScanClk_PHASE_MASK;
}
