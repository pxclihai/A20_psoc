      ...
      
{
    ...
    if(NULL != wrData)
    {
        ...
        else if(I2CM_SM_MSTR_HALT == I2CM_state)
        {
            ...
            CyIntClearPending(I2CM_ISR_NUMBER);
        ...
        if(I2CM_MSTR_NO_ERROR == errStatus)
        {
            ...
            I2CM_mstrControl = mode;    /* Save transaction mode */
            ...
            I2CM_EnableInt();   /* Enable intr to complete transfer */
        }
    }
      ...
      
{
    ...
    if(I2CM_SM_IDLE == I2CM_state)
    {
        ...
        if(I2CM_CHECK_BUS_FREE(I2CM_MCSR_REG))
        {
            I2CM_DisableInt();  /* Disable ISR for Manual functions */

            slaveAddress = ((uint8) (slaveAddress << I2CM_SLAVE_ADDR_SHIFT)); /* Set Address */
            ...
            I2CM_DATA_REG = slaveAddress;   /* Write address to data reg */
        ...
        {
            errStatus = I2CM_MSTR_BUS_BUSY; /* Bus is busy */
        }
    }
...
uint8 I2CM_MasterSendStop(void) 
{
    ...
    if(I2CM_CHECK_MASTER_MODE(I2CM_MCSR_REG))
    {
        I2CM_GENERATE_STOP_MANUAL;              /* Generate STOP */
        I2CM_state = I2CM_SM_IDLE;  /* Reset state to IDLE */
...
uint8 I2CM_MasterWriteByte(uint8 theByte) 
{
    ...
    if(I2CM_CHECK_MASTER_MODE(I2CM_MCSR_REG))
    {
        ...
        I2CM_state = I2CM_SM_MSTR_WR_DATA;  /* Set state WR_DATA */
        ...
        while(I2CM_WAIT_BYTE_COMPLETE(I2CM_CSR_REG))
        {
            ...
            {
                I2CM_state = I2CM_SM_MSTR_HALT;     /* Set state to HALT */
                errStatus = I2CM_MSTR_NO_ERROR;                 /* The LRB was ACKed */
            }
            else
            {
                I2CM_state = I2CM_SM_MSTR_HALT;     /* Set state to HALT */
                errStatus = I2CM_MSTR_ERR_LB_NAK;               /* The LRB was NACKed */
            }
    }
