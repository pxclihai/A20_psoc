...
static uint8 XDATA I2CM_slReadBuf[I2CM_BTLDR_SIZEOF_READ_BUFFER];

/* I2C read buffer: Host reads responses from it */
static uint8 XDATA I2CM_slWriteBuf[I2CM_BTLDR_SIZEOF_WRITE_BUFFER];
...
{
    /* Set Write buffer */
    I2CM_SlaveInitWriteBuf(I2CM_slWriteBuf, I2CM_BTLDR_SIZEOF_WRITE_BUFFER);

    /* Set Read buffer which has zero elements */
    I2CM_SlaveInitReadBuf(I2CM_slReadBuf, 0u);
...
*
*******************************************************************************/
cystatus I2CM_CyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut) CYSMALL
         ...
         
{
    ...
    if((NULL != pData) && (size > 0u))
    {
        ...
        while(0u != timeoutMs)  /* Wait for command from the host */
        {
            ...
            if(0u != (I2CM_slStatus & I2CM_SSTAT_WR_CMPLT))
            {
                /* How many bytes the host has been written */
                byteCount = I2CM_slWrBufIndex;
                *count = (uint16) byteCount;

                /* Copy command to pData buffer */
                (void) memcpy((void *) pData, (void *) I2CM_slWriteBuf,
                              I2CM_MIN_UNT16(byteCount, size));
