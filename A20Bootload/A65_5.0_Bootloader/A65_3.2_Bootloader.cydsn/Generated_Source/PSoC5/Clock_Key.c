...
void Clock_Key_Start(void) 
{
    ...
    Clock_Key_CLKEN |= Clock_Key_CLKEN_MASK;
	Clock_Key_CLKSTBY |= Clock_Key_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_Key_Stop
********************************************************************************
*
* Summary:
...
void Clock_Key_Stop(void) 
{
    ...
    Clock_Key_CLKEN &= (uint8)(~Clock_Key_CLKEN_MASK);
	Clock_Key_CLKSTBY &= (uint8)(~Clock_Key_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_Key_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_Key_CLKEN & Clock_Key_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_Key_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_Key_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_Key__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_Key_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_Key_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_Key_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_Key_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_Key_GetSourceRegister();
    uint16 oldDivider = Clock_Key_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_Key_CLKEN & Clock_Key_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_Key_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_Key_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_Key_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_Key_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Key_MOD_SRC |= modeBitMask & (uint8)Clock_Key_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Key_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Key_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_Key_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_Key_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Key_MOD_SRC & (uint8)(Clock_Key_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_Key_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_Key_GetDividerRegister();
    uint8 oldSrc = Clock_Key_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_Key_MOD_SRC |= CYCLK_SSS;
        Clock_Key_MOD_SRC =
            (Clock_Key_MOD_SRC & (uint8)(~Clock_Key_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_Key_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Key_MOD_SRC & Clock_Key_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_Key_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Key_PHASE = clkPhase & Clock_Key_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Key_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_Key_PHASE & Clock_Key_PHASE_MASK;
}
